...
void TerminalDisplay::setScreenWindow(ScreenWindow *window) {
  ...
  if (!_screenWindow.isNull()) {
    ...
    connect(_screenWindow.data(),
            &Konsole::ScreenWindow::currentResultLineChanged, this,
            &Konsole::TerminalDisplay::updateImage);
    connect(_screenWindow.data(), &Konsole::ScreenWindow::outputChanged, this,
            [this]() { _filterUpdateRequired = true; });
    connect(_screenWindow.data(), &Konsole::ScreenWindow::screenAboutToChange,
            this, [this]() {
              _iPntSel = QPoint(-1, -1);
              _pntSel = QPoint(-1, -1);
    ...
    auto profile = SessionManager::instance()->sessionProfile(
        _sessionController->session());
    _screenWindow->screen()->setReflowLines(
        profile->property<bool>(Profile::ReflowLines));
    _screenWindow->screen()->setIgnoreWcWidth(
        profile->property<bool>(Profile::IgnoreWcWidth));
    _screenWindow->screen()->setEnableUrlExtractor(
        profile->allowEscapedLinks());

    if (_screenWindow->screen()->urlExtractor()) {
      _screenWindow->screen()->urlExtractor()->setAllowedLinkSchema(
          profile->escapedLinksSchema());
...
static UCharDirection BiDiClass([[maybe_unused]] const void *context,
                                UChar32 c) {
...
/**
 * This function installs the factory function which lets Qt instantiate the
 * QAccessibleInterface for the TerminalDisplay.
 */
...
TerminalDisplay::TerminalDisplay(QWidget *parent)
    : QWidget(parent), _verticalLayout(new QVBoxLayout(this)),
      _iPntSel(QPoint(-1, -1)), _pntSel(QPoint(-1, -1)),
      _tripleSelBegin(QPoint(-1, -1)),
      ...
      _terminalFont(std::make_unique<TerminalFont>(this)), _id(++lastViewId) {
  ...
  connect(_headerBar, &TerminalHeaderBar::requestToggleExpansion, this,
          &Konsole::TerminalDisplay::requestToggleExpansion);
  connect(_headerBar, &TerminalHeaderBar::requestMoveToNewTab, this,
          [this] { requestMoveToNewTab(this); });
  connect(_scrollBar, &QScrollBar::sliderMoved, this,
          &Konsole::TerminalDisplay::viewScrolledByUser);

  // setup timers for blinking text
  _blinkTextTimer = new QTimer(this);
  _blinkTextTimer->setInterval(TEXT_BLINK_DELAY);
  connect(_blinkTextTimer, &QTimer::timeout, this,
  ...
  // Workaround for QTBUG-88249 (non-flick gestures recognized as accelerating
  // flick)
  ...
  // Keep this last
  CompositeWidgetFocusWatcher *focusWatcher =
      new CompositeWidgetFocusWatcher(this);
  ...
  connect(&_bell, &TerminalBell::visualBell, this,
          [this] { _terminalColor->visualBell(); });

#ifndef QT_NO_ACCESSIBILITY
  QAccessible::installFactory(Konsole::accessibleInterfaceFactory);
#endif

  connect(KonsoleSettings::self(), &KonsoleSettings::configChanged, this,
          &TerminalDisplay::setupHeaderVisibility);

  _terminalColor = new TerminalColor(this);
  connect(_terminalColor, &TerminalColor::onPalette, _scrollBar,
          &TerminalScrollBar::updatePalette);

  _terminalPainter = new TerminalPainter(this);

  auto ldrawBackground = [this](QPainter &painter, const QRect &rect,
                                const QColor &backgroundColor,
                                bool useOpacitySetting) {
    _terminalPainter->drawBackground(painter, rect, backgroundColor,
                                     useOpacitySetting);
  };
  auto ldrawContents = [this](QPainter &paint, const QRect &rect,
                              bool friendly) {
    _terminalPainter->drawContents(_image, paint, rect, friendly, _imageSize,
                                   _bidiEnabled, _lineProperties);
  };
...
void TerminalDisplay::setCursorStyle(Enum::CursorShapeEnum shape,
                                     bool isBlinking,
...
void TerminalDisplay::resetCursorStyle() {
  ...
  Profile::Ptr currentProfile =
      SessionManager::instance()->sessionProfile(_sessionController->session());

  if (currentProfile != nullptr) {
    auto shape = static_cast<Enum::CursorShapeEnum>(
        currentProfile->property<int>(Profile::CursorShape));
...
void TerminalDisplay::scrollScreenWindow(
    enum ScreenWindow::RelativeScrollMode mode, int amount) {
...
void TerminalDisplay::updateImage() {
  ...
  // disable this shortcut for transparent konsole with scaled pixels, otherwise
  // we get rendering artifacts, see BUG 350651
  if (!(WindowSystemInfo::HAVE_TRANSPARENCY &&
        (qApp->devicePixelRatio() > 1.0)) &&
      _wallpaper->isNull() && !_searchBar->isVisible()) {
    ...
    if (!((_outputSuspendedMessageWidget != nullptr) &&
        ...
        !((_readOnlyMessageWidget != nullptr) &&
          _readOnlyMessageWidget->isVisible())) {
      // hide terminal size label to prevent it being scrolled and show again
      // after scroll
      const bool viewResizeWidget =
          (_resizeWidget != nullptr) && _resizeWidget->isVisible();
      if (viewResizeWidget) {
        _resizeWidget->hide();
      }
      _scrollBar->scrollImage(_screenWindow->scrollCount(),
                              _screenWindow->scrollRegion(), _image,
                              _imageSize);
      if (viewResizeWidget) {
        _resizeWidget->show();
      }
  ...
  _scrollBar->setScroll(_screenWindow->currentLine(),
  ...
  for (y = 0; y < linesToUpdate; ++y) {
    ...
    if (!_resizing) { // not while _resizing, we're expecting a paintEvent
      for (x = 0; x < columnsToUpdate; ++x) {
        ...
        if (dirtyMask[x] != 0) {
          ...
          const bool lineDraw =
              LineBlockCharacters::canDraw(newLine[x + 0].character);
          const bool doubleWidth =
              (x + 1 == columnsToUpdate)
                  ? false
                  : newLine[x + 1].isRightHalfOfDoubleWide();
          ...
          for (len = 1; len < lln; ++len) {
            ...
            const bool nextIsDoubleWidth =
                (x + len + 1 == columnsToUpdate)
                    ? false
                    : newLine[x + len + 1].isRightHalfOfDoubleWide();

            if (ch.foregroundColor != cf || ch.backgroundColor != clipboard ||
                (ch.rendition.all & ~RE_EXTENDED_CHAR) !=
                    (cr & ~RE_EXTENDED_CHAR) ||
                (dirtyMask[x + len] == 0) ||
    ...
    if (y >= _lineProperties.count() || y >= newLineProperties.count() ||
        _lineProperties[y] != newLineProperties[y]) {
      updateLine = true;
    }
  ...
  if (linesToUpdate < _usedLines) {
    dirtyRegion |= highdpi_adjust_rect(QRect(
        _contentRect.left() + tLx,
  ...
  if (columnsToUpdate < _usedColumns) {
    dirtyRegion |= highdpi_adjust_rect(
        QRect(_contentRect.left() + tLx +
                  columnsToUpdate * _terminalFont->fontWidth(),
  ...
  QAccessibleTextCursorEvent cursorEvent(
      this, _usedColumns * screenWindow()->screen()->getCursorY() +
  ...
  if ((text.isEmpty() || _showTerminalSizeHint) && isVisible()) {
    if (_resizeWidget == nullptr) {
      ...
      _resizeWidget->setMinimumWidth(
          _resizeWidget->fontMetrics().boundingRect(text).width());
      _resizeWidget->setMinimumHeight(_resizeWidget->sizeHint().height());
      _resizeWidget->setAlignment(Qt::AlignCenter);

      _resizeWidget->setStyleSheet(
          QStringLiteral("background-color:palette(window);border-style:solid;"
    ...
    _resizeWidget->setMinimumWidth(
        _resizeWidget->fontMetrics().boundingRect(text).width() + 16);
...
void TerminalDisplay::paintEvent(QPaintEvent *pe) {
  ...
  // Determine which characters should be repainted (1 region unit = 1
  // character)
  ...
  for (const QRect &rect : region) {
    ...
    // We can use the opacity settings only if we are in a top level window
    // which actually supports opacity. Many apps that use a konsole part such
    // as kate or dolphin don't for performance reasons. This will result in
    // repaint glitches iin wayland due to missing damage information
    const bool useOpacity =
        window() && window()->testAttribute(Qt::WA_TranslucentBackground);
    _terminalPainter->drawBackground(
        paint, rect, _terminalColor->backgroundColor(), useOpacity);
  }
  ...
  paint.setRenderHint(QPainter::TextAntialiasing,
                      _terminalFont->antialiasText());

  for (const QRect &rect : std::as_const(dirtyImageRegion)) {
    _terminalPainter->drawContents(_image, paint, rect, false, _imageSize,
                                   _bidiEnabled, _lineProperties,
                                   _screenWindow->screen()->ulColorTable());
  }

  if (screenWindow()->currentResultLine() != -1) {
    _searchResultRect.setRect(
        0,
        contentRect().top() + (screenWindow()->currentResultLine() -
                               screenWindow()->currentLine()) *
                                  _terminalFont->fontHeight(),
        columns() * terminalFont()->fontWidth(), _terminalFont->fontHeight());
    _searchResultRect = highdpi_adjust_rect(_searchResultRect);
    _terminalPainter->drawCurrentResultRect(paint, _searchResultRect);
  }

  if (_scrollBar->highlightScrolledLines().isEnabled()) {
    _terminalPainter->highlightScrolledLines(
        paint, _scrollBar->highlightScrolledLines().isTimerActive(),
        _scrollBar->highlightScrolledLines().rect());
  }
  _terminalPainter->drawInputMethodPreeditString(paint, preeditRect(),
                                                 _inputMethodData, _image);
  paintFilters(paint);

  const bool drawBorder = _borderWhenActive && hasFocus();
  if (drawBorder) {
    paint.setPen(_focusBorderColor);
    const auto x = _scrollBar->scrollBarPosition() == Enum::ScrollBarLeft
                       ? _scrollBar->width()
                       : 0;
    const auto sb = _scrollBar->scrollBarPosition() != Enum::ScrollBarHidden
                        ? _scrollBar->width()
                        : 0;
  ...
  if (_drawOverlay) {
    const auto y = _headerBar->isVisible() ? _headerBar->height() : 0;
    const auto rect =
        _overlayEdge == Qt::LeftEdge  ? QRect(0, y, width() / 2, height())
        : _overlayEdge == Qt::TopEdge ? QRect(0, y, width(), height() / 2)
        : _overlayEdge == Qt::RightEdge
            ? QRect(width() - width() / 2, y, width() / 2, height())
...
QRect TerminalDisplay::widgetToImage(const QRect &widgetArea) const {
  ...
  result.setLeft(
      qBound(0,
             (widgetArea.left() - contentsRect().left() - _contentRect.left()) /
                 fontWidth,
             _usedColumns - 1));
  result.setTop(
      qBound(0,
             (widgetArea.top() - contentsRect().top() - _contentRect.top()) /
                 fontHeight,
             _usedLines - 1));
  result.setRight(qBound(
      0,
      (widgetArea.right() - contentsRect().left() - _contentRect.left()) /
          fontWidth,
      _usedColumns - 1));
  result.setBottom(
      qBound(0,
             (widgetArea.bottom() - contentsRect().top() - _contentRect.top()) /
                 fontHeight,
             _usedLines - 1));
  return result;
}
...
void TerminalDisplay::focusOutEvent(QFocusEvent *) {
  ...
  // If waiting for a triple click - losing focus cancels that (do the pending
  // copy)
...
void TerminalDisplay::updateCursor() {
  ...
  QRect cursorRect = imageToWidget(
      highdpi_adjust_rect(QRect(_visualCursorPosition, QSize(charWidth, 1))));
  update(cursorRect);
}
...
void TerminalDisplay::resizeEvent(QResizeEvent *event) {
  ...
  const auto scrollBarWidth =
      _scrollBar->scrollBarPosition() != Enum::ScrollBarHidden
          ? _scrollBar->width()
          : 0;
...
void TerminalDisplay::updateImageSize() {
  ...
  if (oldImage != nullptr) {
    ...
    for (int line = 0; line < lines; line++) {
      memcpy((void *)&_image[_columns * line],
             (void *)&oldImage[oldColumns * line], columns * sizeof(Character));
    }
    delete[] oldImage;
  }
  ...
  if (_resizing) {
    ...
    _pntSel = QPoint(-1, -1);
    _tripleSelBegin = QPoint(-1, -1);
    Q_EMIT changedContentSizeSignal(_contentRect.height(),
                                    _contentRect.width()); // expose resizeEvent
  }
...
void TerminalDisplay::calcGeometry() {
  ...
  _contentRect = contentsRect().adjusted(
      _margin + (_scrollBar->highlightScrolledLines().isEnabled()
                     ? _scrollBar->highlightScrolledLines()
  ...
  // and clamp it to MAX_LINE_WIDTH-1 wide to prevent text shaping buffer
  // overflows
  ...
  if (_centerContents) {
    QSize unusedPixels =
        _contentRect.size() -
        QSize(_columns * fontWidth, _lines * _terminalFont->fontHeight());
    _contentRect.adjust(unusedPixels.width() / 2, unusedPixels.height() / 2, 0,
                        0);
  }
}
...
void TerminalDisplay::setSize(int columns, int lines) {
  ...
  QSize newSize = QSize(horizontalMargin + scrollBarWidth +
                            (columns * _terminalFont->fontWidth()),
...
inline int mouseButton(int button, Qt::KeyboardModifiers modifiers) {
  // Modifier keys, we don't support Shift (4), because it's used to bypass
  // Mouse Tracking mode
...
void TerminalDisplay::mousePressEvent(QMouseEvent *ev) {
  ...
  if (_readOnlyMessageWidget != nullptr &&
      _readOnlyMessageWidget->isVisible() &&
      _readOnlyMessageWidget->frameGeometry().contains(ev->pos())) {
    return;
  }
  ...
  auto [charLine, charColumn] =
      getCharacterPosition(ev->pos(), !usesMouseTracking());
  ...
  if (hotSpotClick && hotSpotClick->hasDragOperation() &&
  ...
  if (ev->button() == Qt::LeftButton) {
    ...
    if ((!usesMouseTracking() && !ev->modifiers()) ||
        (usesMouseTracking() && ev->modifiers() == Qt::ShiftModifier)) {
    ...
    if ((!_ctrlRequiredForDrag ||
         ((ev->modifiers() & Qt::ControlModifier) != 0u)) &&
        selected) {
      ...
      _preserveLineBreaks = !(((ev->modifiers() & Qt::ControlModifier) != 0u) &&
                              !(ev->modifiers() & Qt::AltModifier));
      _columnSelectionMode = ((ev->modifiers() & Qt::AltModifier) != 0u) &&
                             ((ev->modifiers() & Qt::ControlModifier) != 0u);

      // There are a couple of use cases when selecting text :
      // Normal buffer or Alternate buffer when not using Mouse Tracking:
      //  select text or extendSelection or columnSelection or columnSelection +
      //  extendSelection
      //
      // Alternate buffer when using Mouse Tracking and with Shift pressed:
      //  select text or columnSelection
      if (!usesMouseTracking() &&
          ((ev->modifiers() == Qt::ShiftModifier) ||
           (((ev->modifiers() & Qt::ShiftModifier) != 0u) &&
        ...
        Q_EMIT mouseSignal(
            mouseButton(0, ev->modifiers()), charColumn + 1,
            charLine + 1 + _scrollBar->value() - _scrollBar->maximum(), 0);
      }
    }
    if (_semanticInputClick && (ev->modifiers() & Qt::ControlModifier) == 0 &&
        _screenWindow->screen()->replMode() == REPL_INPUT) {
      Q_EMIT mouseSignal(
          mouseButton(0, ev->modifiers()), charColumn + 1,
          charLine + 1 + _scrollBar->value() - _scrollBar->maximum(), 3);
    }
    ...
    if (!usesMouseTracking() || ((ev->modifiers() & Qt::ShiftModifier) != 0u)) {
      ...
      if (!_readOnly) {
        Q_EMIT mouseSignal(
            mouseButton(2, ev->modifiers()), charColumn + 1,
            charLine + 1 + _scrollBar->value() - _scrollBar->maximum(), 0);
      }
    }
  }
...
void TerminalDisplay::mouseMoveEvent(QMouseEvent *ev) {
  ...
  if (_possibleTripleClick &&
      (ev->pos() - _tripleClickPos).manhattanLength() > 20) {
  ...
  if (usesMouseTracking() && !(ev->modifiers() & Qt::ShiftModifier)) {
    ...
    if (!_readOnly) {
      ...
      Q_EMIT mouseSignal(
          mouseButton(button, ev->modifiers()), charColumn + 1,
  ...
  if (_dragInfo.state == diPending) {
    ...
    if (ev->position().x() > _dragInfo.start.x() + distance ||
        ev->position().x() < _dragInfo.start.x() - distance ||
        ev->position().y() > _dragInfo.start.y() + distance ||
        ev->position().y() < _dragInfo.start.y() - distance) {
...
void TerminalDisplay::extendSelection(const QPoint &position) {
  ...
  if (_iPntSel.x() < 0 || _iPntSel.y() < 0 || _pntSel.x() < 0 ||
      _pntSel.y() < 0) {
  ...
  QRect textBounds(tLx + _contentRect.left(), tLy + _contentRect.top(),
  ...
  if (oldpos.y() > textBounds.bottom()) {
    linesBeyondWidget =
        (oldpos.y() - textBounds.bottom()) / _terminalFont->fontHeight();
    _scrollBar->setValue(_scrollBar->value() + linesBeyondWidget +
                         1); // scrollforward
  }
  if (oldpos.y() < textBounds.top()) {
    linesBeyondWidget =
        (textBounds.top() - oldpos.y()) / _terminalFont->fontHeight();
    _scrollBar->setValue(_scrollBar->value() - linesBeyondWidget -
                         1); // history
  }
  ...
  if (_wordSelectionMode) {
    ...
    const bool left_not_right =
        (here.y() < iPntSelCorr.y() ||
         (here.y() == iPntSelCorr.y() && here.x() < iPntSelCorr.x()));
    const bool old_left_not_right = (pntSelCorr.y() < iPntSelCorr.y() ||
                                     (pntSelCorr.y() == iPntSelCorr.y() &&
                                      pntSelCorr.x() < iPntSelCorr.x()));
    ...
    if (left.y() < 0 || left.y() >= _lines || left.x() < 0 ||
        left.x() >= _columns) {
      left = pntSelCorr;
    } else {
      left = findWordStart(left);
    }
    if (right.y() < 0 || right.y() >= _lines || right.x() < 0 ||
        right.x() >= _columns) {
      right = pntSelCorr;
    } else {
...
void TerminalDisplay::mouseReleaseEvent(QMouseEvent *ev) {
      ...
      if (_actSel > 1) {
        ...
        if (_possibleTripleClick) {
          ...
          if (!text.isEmpty()) {
            ...
            if (_copyTextAsHTML) {
              _doubleClickSelectedHtml = _screenWindow->selectedText(
                  currentDecodingOptions() | Screen::ConvertToHtml);
            }
          }
        }
      }
  ...
  if (usesMouseTracking() && !_readOnly &&
      (ev->button() == Qt::RightButton || ev->button() == Qt::MiddleButton) &&
      !(ev->modifiers() & Qt::ShiftModifier)) {
    Q_EMIT mouseSignal(
        mouseButton((ev->button() == Qt::MiddleButton ? 1 : 2),
                    ev->modifiers()),
        charColumn + 1,
        charLine + 1 + _scrollBar->value() - _scrollBar->maximum(), 2);
  }
...
QPair<int, int> TerminalDisplay::getCharacterPosition(const QPoint &widgetPoint,
                                                      bool edge) const {
  ...
  int line =
      qBound(0,
             (widgetPoint.y() - contentsRect().top() - _contentRect.top()) /
                 _terminalFont->fontHeight(),
             _usedLines - 1);
  bool doubleWidth = line < _lineProperties.count() &&
                     _lineProperties[line].flags.f.doublewidth;
  bool shaped;
  int column = qBound(0,
                      (widgetPoint.x() + xOffset - contentsRect().left() -
                       _contentRect.left()) /
                          _terminalFont->fontWidth() / (doubleWidth ? 2 : 1),
                      columnMax);
  ...
  if (_bidiEnabled && column < _usedColumns) {
    ...
    QString line;
    bidiMap(_image + pos, line, log2line, line2log, shapemap, vis2line, shaped,
            false);
...
void TerminalDisplay::processMidButtonClick(QMouseEvent *ev) {
  if (!usesMouseTracking() || ((ev->modifiers() & Qt::ShiftModifier) != 0u)) {
    ...
    // If currently waiting for a triple click, a middle click cancels that -
    // copy now
    copyToX11Selection(true);
    ...
    if (!_readOnly) {
      auto [charLine, charColumn] =
          getCharacterPosition(ev->pos(), !usesMouseTracking());
      Q_EMIT mouseSignal(
          mouseButton(1, ev->modifiers()), charColumn + 1,
          charLine + 1 + _scrollBar->value() - _scrollBar->maximum(), 0);
    }
  }
}
...
void TerminalDisplay::mouseDoubleClickEvent(QMouseEvent *ev) {
  ...
  if (usesMouseTracking() && !(ev->modifiers() & Qt::ShiftModifier)) {
    if (!_readOnly) {
      ...
      Q_EMIT mouseSignal(
          mouseButton((ev->button() == Qt::LeftButton ? 0 : 2),
                      ev->modifiers()),
          charColumn + 1,
          charLine + 1 + _scrollBar->value() - _scrollBar->maximum(), 0);
    }
  ...
  QTimer::singleShot(QApplication::doubleClickInterval(), this, [this]() {
    _possibleTripleClick = false;
    copyToX11Selection(
        true); // this will do nothing if another copy happened in the meantime
  });
}
...
void TerminalDisplay::wheelEvent(QWheelEvent *ev) {
  ...
  if (((modifiers & Qt::ControlModifier) != 0u) && _mouseWheelZoom &&
    ...
    int steps = _scrollWheelState.consumeLegacySteps(
        ScrollState::DEFAULT_ANGLE_SCROLL_LINE);
    for (; steps > 0; --steps) {
    ...
    if (!usesMouseTracking() &&
        !_sessionController->session()->isPrimaryScreen() &&
        _scrollBar->alternateScrolling()) {
      // Send simulated up / down key presses to the terminal program
      // for the benefit of programs such as 'less' (which use the alternate
      // screen)
      ...
      const int lines = _scrollWheelState.consumeSteps(
          static_cast<int>(_terminalFont->fontHeight() *
                           qApp->devicePixelRatio()),
          ScrollState::degreesToAngle(5));
      ...
      auto [charLine, charColumn] =
          getCharacterPosition(ev->position().toPoint(), !usesMouseTracking());
      const int steps = _scrollWheelState.consumeLegacySteps(
          ScrollState::DEFAULT_ANGLE_SCROLL_LINE);
      const int button = (steps > 0) ? 64 : 65;
      for (int i = 0; i < abs(steps); ++i) {
        // Alt+wheel unsupported, Qt transforms it into horizontal wheel, see
        // QTBUG-30948
        Q_EMIT mouseSignal(
            mouseButton(button, ev->modifiers()), charColumn + 1,
            charLine + 1 + _scrollBar->value() - _scrollBar->maximum(), 0);
...
QPoint TerminalDisplay::findLineStart(const QPoint &pnt) {
  ...
  while (lineInHistory > 0) {
    ...
    lineProperties =
        screen->getLineProperties(newRegionStart, lineInHistory - 1);
...
QPoint TerminalDisplay::findLineEnd(const QPoint &pnt) {
  ...
  while (lineInHistory < maxY) {
    for (; line < lineProperties.count() && lineInHistory < maxY;
         line++, lineInHistory++) {
    ...
    line = 0;
    lineProperties = screen->getLineProperties(
        lineInHistory, qMin(lineInHistory + visibleScreenLines, maxY));
  }
  return {_columns - 1, lineInHistory - topVisibleLine};
}
...
QPoint TerminalDisplay::findWordEnd(const QPoint &pnt) {
  ...
  if (hotspot) {
    ...
    // It really should be fixed in filters, but this is the best we have until
    // then
    if (col > 0) {
      col--;
    } else {
    ...
    for (;; j++, x++) {
      if (x < maxX) {
        if (charClass(image[j + 1]) == selClass &&
            ...
            !(image[j + 1].character == ':' &&
              charClass(image[j + 2]) == QLatin1Char(' '))) {
          continue;
        }
        goto out;
      } else if (i < lineCount - 1) {
        if (((lineProperties[i].flags.f.wrapped) != 0) &&
...
out:
  ...
  if ((image[j].rendition.f.extended == 0) && (image[j].character == '@') &&
      (y > pnt.y() || x > pnt.x())) {
    if (x > 0) {
      x--;
    } else {
...
bool TerminalDisplay::isInTerminalRegion(const QPoint &point) const {
  ...
  return !(!visibleRegion().contains(point) ||
           _scrollBar->frameGeometry().contains(point) ||
           inMessageSuspendedWidget);
}
...
void TerminalDisplay::mouseTripleClickEvent(QMouseEvent *ev) {
  ...
  if (_screenWindow->screen()->hasRepl() &&
      ev->modifiers() & Qt::ControlModifier) {
    _screenWindow->screen()->selectReplContigious(
        charColumn, charLine + _screenWindow->currentLine());
    copyToX11Selection();
  } else {
    selectLine(QPoint(charColumn, charLine),
               _tripleClickMode == Enum::SelectWholeLine, true);
  }
}
...
void TerminalDisplay::selectLine(QPoint pos, bool entireLine,
  ...
  if (!entireLine) { // Select from cursor to end of line
    ...
    _screenWindow->setSelectionStart(_tripleSelBegin.x(), _tripleSelBegin.y(),
                                     false);
  } else {
...
uint TerminalDisplay::charClass(const Character &ch) const {
  if (ch.rendition.f.extended != 0) {
    ...
    const char32_t *chars = ExtendedCharTable::instance.lookupExtendedChar(
        ch.character, extendedCharLength);
    ...
    if (QChar::isLetterOrNumber(ch.character) ||
        _wordCharacters.contains(QString::fromUcs4(&ch.character, 1),
                                 Qt::CaseInsensitive)) {
      return 'a';
    }
...
void TerminalDisplay::scrollPrepareEvent(QScrollPrepareEvent *event) {
  ...
  const int lineHeight =
      _terminalFont->fontHeight() + _terminalFont->lineSpacing();
  // clang-format off
  ...
  // Abandon the scroller if it's still pressed and not being dragged after
  // 500ms
...
void TerminalDisplay::scrollEvent(QScrollEvent *event) {
  ...
  if (linesScrolled != 0) {
    scrollScreenWindow(ScreenWindow::RelativeScrollMode::ScrollLines,
                       linesScrolled);
  }
...
void TerminalDisplay::doPaste(QString text, bool appendReturn) {
  ...
  if (text.length() > 8000) {
    if (KMessageBox::warningContinueCancel(
            ...
            i18np("Are you sure you want to paste %1 character?",
                  ...
                  text.length()),
            i18n("Confirm Paste"), KStandardGuiItem::cont(),
  ...
  static const auto isUnsafe = [](const QChar &c) {
    return (c.category() == QChar::Category::Other_Control &&
            !whitelist.contains(c.unicode()));
  };
  ...
  for (const QChar &c : text) {
    if (isUnsafe(c)) {
      ...
      const QString description =
          characterDescriptions.value(c.unicode(), QString());
      QString entry = QStringLiteral("U+%1").arg(static_cast<uint>(c.unicode()),
                                                 4, 16, QLatin1Char('0'));
      if (!sequence.isEmpty()) {
  ...
  if (!unsafeCharacters.isEmpty()) {
    int result = KMessageBox::warningTwoActionsCancelList(
        window(),
        i18n("The text you're trying to paste contains hidden control "
             "characters, "
             "do you want to filter them out?"),
        unsafeCharacters, i18nc("@title", "Confirm Paste"),
        KGuiItem(i18nc("@action:button", "Paste &without control characters"),
                 QStringLiteral("filter-symbolic")),
        KGuiItem(i18nc("@action:button", "&Paste everything"),
...
void TerminalDisplay::setMiddleClickPasteMode(
    Enum::MiddleClickPasteModeEnum mode) {
...
void TerminalDisplay::copyToX11Selection(bool useSavedText) {
  ...
  if (useSavedText) {
    ...
    if (!text.isEmpty() && _copyTextAsHTML) {
      html = _screenWindow->selectedText(currentDecodingOptions() |
                                         Screen::ConvertToHtml);
    }
  }
...
void TerminalDisplay::copyToClipboard(Screen::DecodingOptions options) {
  ...
  const QString &text =
      _screenWindow->selectedText(currentDecodingOptions() | options);
  if (text.isEmpty()) {
    return;
  }
  ...
  if (_copyTextAsHTML) {
    mimeData->setHtml(_screenWindow->selectedText(currentDecodingOptions() |
                                                  Screen::ConvertToHtml));
  }
...
void TerminalDisplay::pasteFromClipboard(bool appendEnter) {
  ...
  // - paste the path(s) as a space-separated list of strings, which are quoted
  // if needed
  if (!mimeData->hasUrls()) { // fast path if there are no urls
    ...
    for (const QUrl &url : list) {
      if (url.isLocalFile()) {
        ...
        text += QLatin1Char(' ');
      } else { // can users copy urls of both local and remote files at the same
               // time?
        text = mimeData->text();
        break;
      }
...
QRect TerminalDisplay::preeditRect() const {
  const int preeditLength =
      Character::stringWidth(_inputMethodData.preeditString);

  if (preeditLength == 0) {
    return {};
  }
  const QRect stringRect(
      _contentRect.left() + _terminalFont->fontWidth() * cursorPosition().x(),
      _contentRect.top() + _terminalFont->fontHeight() * cursorPosition().y(),
      _terminalFont->fontWidth() * preeditLength, _terminalFont->fontHeight());
  ...
  if (_outputSuspendedMessageWidget == nullptr) {
    ...
    _outputSuspendedMessageWidget =
        createMessageWidget(i18n("<qt>Output has been "
                                 ...
                                 "Software_flow_control\">suspended</a>"
                                 " by pressing Ctrl+S."
                                 " Press <b>Ctrl+Q</b> to resume.</qt>"));

    connect(_outputSuspendedMessageWidget, &KMessageWidget::linkActivated, this,
            [](const QString &url) { QDesktopServices::openUrl(QUrl(url)); });

    _outputSuspendedMessageWidget->setMessageType(KMessageWidget::Warning);
  }

  suspended ? _outputSuspendedMessageWidget->animatedShow()
            : _outputSuspendedMessageWidget->animatedHide();
}
  ...
  if (readonly) {
    ...
    if (_readOnlyMessageWidget == nullptr) {
      _readOnlyMessageWidget =
          createMessageWidget(i18n("This terminal is read-only."));
      _readOnlyMessageWidget->setIcon(
          QIcon::fromTheme(QStringLiteral("object-locked")));
    }
  }

  if (_readOnlyMessageWidget != nullptr) {
    readonly ? _readOnlyMessageWidget->animatedShow()
             : _readOnlyMessageWidget->animatedHide();
  }
...
void TerminalDisplay::keyPressEvent(QKeyEvent *event) {
  ...
  if (session()->getSelectMode()) {
    ...
    case Qt::Key_H:
      ...
      if (histLines + y < screenWindow()->currentLine()) {
        scrollScreenWindow(ScreenWindow::RelativeScrollMode::ScrollLines,
                           histLines + y - screenWindow()->currentLine());
      }
      break;
    case Qt::Key_Up:
    case Qt::Key_K:
    ...
    case Qt::Key_D:
      if (event->modifiers() & Qt::ControlModifier) {
        ...
        if (histLines + y >=
            screenWindow()->currentLine() + screen->getLines()) {
          scrollScreenWindow(ScreenWindow::RelativeScrollMode::ScrollLines,
                             histLines + y - screenWindow()->currentLine() -
                                 screen->getLines() + 1);
        }
  ...
  {
    auto [charLine, charColumn] = getCharacterPosition(
        mapFromGlobal(QCursor::pos()), !usesMouseTracking());
  ...
  if (!_peekPrimaryShortcut.isEmpty() &&
      _peekPrimaryShortcut.matches(
  ...
  if (event->modifiers() & Qt::MetaModifier) {
    event->setModifiers((event->modifiers() & ~Qt::MetaModifier) |
                        Qt::ControlModifier);
  ...
  if (!_readOnly) {
    if (!_possibleTripleClick) {
      _actSel = 0; // Key stroke implies a screen update, so TerminalDisplay
                   // won't know where the current selection is.
    }
...
#ifndef QT_NO_ACCESSIBILITY
  if (!_readOnly) {
    QAccessibleTextCursorEvent textCursorEvent(
        this, _usedColumns * screenWindow()->screen()->getCursorY() +
                  screenWindow()->screen()->getCursorX());
...
void setupCdToUrlAction(const QString &dropText, const QUrl &url,
  ...
  const QByteArray triggerText =
      QString(QLatin1String(" cd ") + dropText + QLatin1Char('\n'))
          .toLocal8Bit();
  display->connect(
      cdAction, &QAction::triggered, display,
      [display, triggerText] { Q_EMIT display->sendStringToEmu(triggerText); });
...
void TerminalDisplay::dropEvent(QDropEvent *event) {
  ...
  const bool canBePasted = mimeData->hasFormat(QStringLiteral("text/plain")) ||
                           mimeData->hasFormat(QStringLiteral("text/uri-list"));
  ...
  if (!urls.isEmpty()) {
    ...
    // If our target is local we will open a popup - otherwise the fallback
    // kicks in and the URLs will simply be pasted as text.
    if (!_dropUrlsAsText && (_sessionController != nullptr) &&
        _sessionController->url().isLocalFile()) {
...
IncrementalSearchBar *TerminalDisplay::searchBar() const { return _searchBar; }

void TerminalDisplay::applyProfile(const Profile::Ptr &profile) {
  // load color scheme
  _colorScheme = ViewManager::colorSchemeForProfile(profile);
  _terminalColor->applyProfile(profile, _colorScheme,
  ...
  _scrollBar->setScrollBarPosition(Enum::ScrollBarPositionEnum(
      profile->property<int>(Profile::ScrollBarPosition)));
  _scrollBar->setScrollFullPage(
      profile->property<bool>(Profile::ScrollFullPage));
  ...
  _tripleClickMode = Enum::TripleClickModeEnum(
      profile->property<int>(Profile::TripleClickMode));
  ...
  _trimLeadingSpaces =
      profile->property<bool>(Profile::TrimLeadingSpacesInSelectedText);
  _trimTrailingSpaces =
      profile->property<bool>(Profile::TrimTrailingSpacesInSelectedText);
  _openLinksByDirectClick =
      profile->property<bool>(Profile::OpenLinksByDirectClickEnabled);
  setMiddleClickPasteMode(Enum::MiddleClickPasteModeEnum(
      profile->property<int>(Profile::MiddleClickPasteMode)));
  ...
  _scrollBar->setHighlightScrolledLines(
      profile->property<bool>(Profile::HighlightScrolledLines));
  ...
  setKeyboardCursorShape(
      Enum::CursorShapeEnum(profile->property<int>(Profile::CursorShape)));
  ...
  // bell mode
  _bell.setBellMode(
      Enum::BellModeEnum(profile->property<int>(Profile::BellMode)));
  ...
  _scrollBar->setAlternateScrolling(
      profile->property<bool>(Profile::AlternateScrolling));
  ...
  _scrollBar->setSearchHighlightLineOpacity(
      profile->property<int>(Profile::SearchLineOpacity));
  _scrollBar->setSearchHighlightLineColor(
      profile->property<QColor>(Profile::SearchLineColor));
  _dimValue = profile->dimValue();
  _focusBorderColor = profile->focusBorderColor();

  _filterChain->setUrlHintsModifiers(Qt::KeyboardModifiers(
      profile->property<int>(Profile::UrlHintsModifiers)));
  _filterChain->setReverseUrlHints(
      profile->property<bool>(Profile::ReverseUrlHints));
...
void TerminalDisplay::printScreen() {
  auto lprintContent = [this](QPainter &painter, bool friendly) {
    ...
    _printManager->printContent(painter, friendly, columnLines, lfontget,
                                lfontset);
  };
...
int TerminalDisplay::selectionState() const { return _actSel; }
...
int TerminalDisplay::bidiMap(Character *screenline, QString &line,
                             int *log2line, int *line2log, uint16_t *shapemap,
                             int32_t *vis2line, bool &shaped, bool shape,
                             bool bidi) const {
  // screenline is the input line: the nth element is the nth character in the
  // (logical) line in UCS4 encoding. line is a translation of screenline to
  // Utf-16 for ICU library UTF-16 characters are not of constant length (and
  // there are konsole's extended characters as well) log2line and line2log
  // translate positions in screenline to positions in line (and reverse)
  ...
  for (i = 0; i < linewidth; i++) {
    ...
    if (char_value.rendition.f.extended != 0) {
      ...
      ushort extendedCharLength = 0;
      if (const char32_t *chars =
              ExtendedCharTable::instance.lookupExtendedChar(
                  char_value.character, extendedCharLength)) {
  ...
  if (shape) {
    ...
    if (lineLastNonSpace > 0 &&
        line[lineLastNonSpace - 1].unicode() == 0x0644 &&
        (line[lineLastNonSpace].unicode() == 0x0627 ||
         line[lineLastNonSpace].unicode() == 0x0625 ||
         line[lineLastNonSpace].unicode() == 0x0623 ||
    ...
    u_shapeArabic(reinterpret_cast<const UChar *>(line.utf16()), line.length(),
                  shaped_line, MAX_LINE_WIDTH,
                  U_SHAPE_AGGREGATE_TASHKEEL_NOOP |
                      U_SHAPE_LENGTH_FIXED_SPACES_NEAR | U_SHAPE_LETTERS_SHAPE,
                  &errorCode);
    for (int i = 0; i < line.length(); i++) {
      if (line[i] != shaped_line[i]) {
        shaped = true;
        if (i < line.length() - 1 && line[i].unicode() == 0x0644 &&
            (line[i + 1].unicode() == 0x0627 ||
             line[i + 1].unicode() == 0x0625 ||
             line[i + 1].unicode() == 0x0623 ||
             line[i + 1].unicode() == 0x0622) &&
            ((shaped_line[i] == 0x0020) != (shaped_line[i + 1] == 0x0020))) {
          switch (shaped_line[i] == 0x0020 ? shaped_line[i + 1]
                                           : shaped_line[i]) {
          case 0x0fefc:
  ...
  if (_bidiTableDirOverride) {
    ubidi_setClassCallback(ubidi, BiDiClass, nullptr, nullptr, nullptr,
                           &errorCode);
  }
  ubidi_setPara(ubidi, reinterpret_cast<const UChar *>(line.utf16()),
                line.length(), paraLevel, nullptr, &errorCode);
  ...
  int p = 0;
  for (int i = 0; i < len; i++) {
    if ((notSkipped[semi_vis2line[i] / 32] &
         (1ul << (semi_vis2line[i] % 32))) != 0) {
