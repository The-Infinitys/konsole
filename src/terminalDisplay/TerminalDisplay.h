 ...
 * compensate in case of approximation issues on highdpi displays with
 * fractional scaling applied.
 */
 ...
 *
 * When the terminal emulation receives new output from the program running in
 * the terminal, it will update the display by calling updateImage().
 *
...
public:
  ...
  int usedColumns() const { return _usedColumns; }
  ...
  TerminalHeaderBar *headerBar() const { return _headerBar; }
   ...
   * rendered by the TerminalDisplay.
   */
  void setScreenWindow(ScreenWindow *window);
  /** Returns the terminal screen section which is displayed in this widget. See
   * setScreenWindow() */
  QPointer<ScreenWindow> screenWindow() const { return _screenWindow; }
  ...
  bool hasCompositeFocus() const { return _hasCompositeFocus; }
  ...
  QPair<int, int> getCharacterPosition(const QPoint &widgetPoint,
                                       bool edge) const;

  // toggle the header bar Minimize/Maximize button.
  void setExpandedMode(bool expand);

  TerminalScrollBar *scrollBar() const { return _scrollBar; }

  TerminalColor *terminalColor() const { return _terminalColor; }

  TerminalFont *terminalFont() const { return _terminalFont.get(); }
  ...
  bool cursorBlinking() const { return _cursorBlinking; }

  bool textBlinking() const { return _textBlinking; }

  Enum::CursorShapeEnum cursorShape() const { return _cursorShape; }

  bool bidiEnabled() const { return _bidiEnabled; }

  bool semanticUpDown() const { return _semanticUpDown; }

  ColorSchemeWallpaper::Ptr wallpaper() const { return _wallpaper; }
  ...
  // returns 0 - not selecting, 1 - pending selection (button pressed but no
  // movement yet), 2 - selecting
  int selectionState() const;

  bool trimTrailingWhitespace() const { return _trimTrailingSpaces; }

  Qt::Edge droppedEdge() const { return _overlayEdge; }
   ...
   */
  void scrollScreenWindow(enum ScreenWindow::RelativeScrollMode mode,
                          int amount);
  ...
  bool getReadOnly() const { return _readOnly || _selectMode; }

  // Get mapping between visual and logical positions in line
  // returns the index of the last non space character.
  int bidiMap(Character *screenline, QString &line, int *log2line,
              int *line2log, uint16_t *shapemap, int32_t *vis2line,
              bool &shaped, bool shape = true, bool bidi = true) const;
...
public Q_SLOTS:
  ...
  /**
   * Changes whether the flow control warning box should be shown when the flow
   * control stop key (Ctrl+S) are pressed.
   */
...
Q_SIGNALS:
   ...
   * for right button, 3 for release, 4 for Shift, 8 for Alt, 16 for Meta, 64
   * for wheel up, 65 for wheel down)
...
private:
   ...
   * @return true if the given point is in the area with the main terminal
   * content and not in some other widget.
   */
  bool isInTerminalRegion(const QPoint &pnt) const;

  // Uses the current settings for trimming whitespace and preserving linebreaks
  // to create a proper flag value for Screen
  ...
  int _usedLines =
      1; // the number of lines that are actually being used, this will be less
  // than 'lines' if the character image provided with setImage() is smaller
  // than the maximum image size which can be displayed

  int _usedColumns = 1; // the number of columns that are actually being used,
                        // this will be less
  ...
  bool _openLinksByDirectClick =
      false; // Open URL and hosts by single mouse click

  bool _ctrlRequiredForDrag = true; // require Ctrl key for drag selected text
  bool _dropUrlsAsText =
      false; // always paste URLs as text without showing copy/move menu

  Enum::TripleClickModeEnum _tripleClickMode = Enum::SelectWholeLine;
  bool _possibleTripleClick =
      false;              // is set in mouseDoubleClickEvent and cleared
                          // after QApplication::doubleClickInterval() delay
  QPoint _tripleClickPos; // The position where a potential triple click was
                          // started
  QString _doubleClickSelectedText; // selected text whose copying may be
                                    // cancelled by further events; copying
  QString _doubleClickSelectedHtml; // is delayed to prevent a triple-click from
                                    // generating > 1 entries in the clipboard
                                    // history (a triple click is a double click
                                    // at first ;)
  ...
  // widgets related to the warning message that appears when the user presses
  // Ctrl+S to suspend terminal output - informing them what has happened and
  // how to resume output
  ...
  KMessageWidget *_readOnlyMessageWidget =
      nullptr; // Message shown at the top when read-only mode gets activated
  ...
  // The Infinity's
  QRectF currentCursorRect; // 現在描画されているアニメーション中の四角形
  QRectF targetCursorRect;  // ターミナルが本来指示している目標位置
  QTimer *animationTimer;   // 毎フレーム更新用のタイマー
};
